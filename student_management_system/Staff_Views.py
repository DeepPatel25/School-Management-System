...
from app.models import (Attendance, Attendance_Report, Session_Year, Staff, Staff_Feedback,
                        Staff_Leave, Staff_Notification, Student, Student_Result,
                        Subject)
...
def STAFF_NOTIFICATION_MARK_AS_DONE(request, staff_notification_id):
    notification = Staff_Notification.objects.get(
...
def STAFF_TAKE_ATTENDANCE(request):
    ...
    if action is not None:
        if request.method == "POST":
            ...
            for i in subject:
                student_id = i.standard.id
                students = Student.objects.filter(standard_id=student_id)
...
def STAFF_SAVE_RESULT(request):
    if request.method == "POST":
        ...
        check_exist = Student_Result.objects.filter(
            subject_id=get_subject,
